...
public class AsientoController {
    ...
    /**
    * Elimina un asiento específico de un teatro.
    * @param teatroId ID del teatro.
    * @param asientoId ID del asiento a eliminar.
    * @return Respuesta con código 204 si se elimina correctamente, 404 si no existe.
    */
    @DeleteMapping("/{asientoId}")
        public ResponseEntity<Void> eliminarAsiento(@PathVariable Long teatroId, @PathVariable Long asientoId) {
     try {
         asientoService.eliminarAsiento(teatroId, asientoId);
            return ResponseEntity.noContent().build(); // 204 No Content si se elimina correctamente
        } catch (EntityNotFoundException e) {
            return ResponseEntity.notFound().build(); // 404 Not Found si no se encuentra el asiento
        } catch (Exception e) {
