...
class PagoServiceTest {
    ...
    private void insertData() {
        for (int i = 0; i < 2; i++) { // Insertar dos pagos exitosos
            PagoEntity pagoEntity = factory.manufacturePojo(PagoEntity.class);
            pagoEntity.setMetodoPago("Tarjeta de Crédito");
            pagoEntity.setMonto(50000.0); // Monto mayor a 0 (exitoso)
            entityManager.persist(pagoEntity);
            entityManager.flush(); // Asegura que JPA asigne un ID
            pagoList.add(pagoEntity); // Ahora tendrá un ID válido
        }
    
        PagoEntity pagoNoExitoso = factory.manufacturePojo(PagoEntity.class);
        pagoNoExitoso.setMetodoPago("Tarjeta de Crédito");
        pagoNoExitoso.setMonto(0.0); // Monto = 0 (no exitoso)
        entityManager.persist(pagoNoExitoso);
        entityManager.flush(); // Asegura que JPA asigne un ID
        pagoList.add(pagoNoExitoso);
    }
    ...
    /**
     * Prueba para verificar un pago exitoso.
     */
    @Test
    @Transactional
    void testVerificarPagoExitoso() throws EntityNotFoundException {
        List<PagoEntity> pagos = pagoRepository.findAll();
        assertFalse(pagos.isEmpty(), "No se insertaron pagos en la BD");
    
        PagoEntity pagoExitoso = pagos.get(0); // Tomar un pago existente de la BD
        boolean resultado = pagoService.verificarPagoExitoso(pagoExitoso.getId());
    
        assertTrue(resultado, "El pago con monto > 0 debería ser exitoso");
    }

    /**
     * Prueba para verificar un pago no exitoso.
     */
    @Test
    void testVerificarPagoNoExitoso() throws EntityNotFoundException {
        PagoEntity pagoNoExitoso = pagoList.get(2);
        boolean resultado = pagoService.verificarPagoExitoso(pagoNoExitoso.getId());
        assertFalse(resultado, "El pago con monto 0 no debería ser exitoso");
    }

    /**
     * Prueba para verificar un pago que no existe.
     */
    @Test
    void testVerificarPagoNoExistente() {
        assertThrows(EntityNotFoundException.class, () -> pagoService.verificarPagoExitoso(999L),
                "Debería lanzar EntityNotFoundException para pagos no existentes");
    }
