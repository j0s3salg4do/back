...
public class AsientoService {
    ...
    @Transactional
    public AsientoEntity crearAsiento(Long teatroId, AsientoEntity asiento) throws EntityNotFoundException {
    log.info("Inicia proceso de creación de asiento para el teatro con id={}", teatroId);

    TeatroEntity teatro = teatroRepository.findById(teatroId)
            .orElseThrow(() -> new EntityNotFoundException("El teatro con id " + teatroId + " no existe."));

    asiento.setTeatro(teatro); // Asociar el asiento con el teatro
    AsientoEntity asientoGuardado = asientoRepository.save(asiento);

    log.info("Finaliza proceso de creación de asiento con id={}", asientoGuardado.getId());
    ...
    public List<AsientoEntity> getAsientos(Long teatroId) throws EntityNotFoundException
    {
        ...
        Optional<TeatroEntity> teatroEntity = teatroRepository.findById(teatroId);
        if (teatroEntity.isEmpty())
        {
            throw new EntityNotFoundException("No se encontró el teatro con Id = " + teatroId);
        }
        return teatroEntity.get().getAsientos();
    }
    ...
    @Transactional
    public void eliminarAsiento(Long teatroId, Long asientoId) throws EntityNotFoundException {
    AsientoEntity asiento = asientoRepository.findById(asientoId)
            .orElseThrow(() -> new EntityNotFoundException("El asiento con ID " + asientoId + " no existe."));
    
    if (!asiento.getTeatro().getId().equals(teatroId)) {
       throw new EntityNotFoundException("El asiento con ID " + asientoId + " no pertenece al teatro con ID " + teatroId);
    }
